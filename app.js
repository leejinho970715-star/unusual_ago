...
function readSavedCart() {
    try {
        ...
        return Array.isArray(saved) ? saved.map(item => PRODUCTS_CATALOG.find(p => p.id === item?.id)).filter(Boolean) : [];
    } catch { return []; }
}
function saveCart() {
    try { localStorage.setItem('type16_cart', JSON.stringify(cart)); } catch { /* Bag remains usable when storage is unavailable. */ }
}
...
function initLookbookTrack() {
    ...
    const items = [
        { mbti: 'ENTP', title: 'COBALT REBEL', img: 'assets/editorial-cobalt.jpg', desc: '코발트 재킷 & 실버 와이드 카고' },
        { mbti: 'INTJ', title: 'DARK MINIMAL TECH', img: 'assets/lookbook_darktech.jpg', desc: '구조적 파워 블레이저 & 사이버 글래스' },
        { mbti: 'INFP', title: 'SOFT NOISE', img: 'assets/editorial-soft.jpg', desc: '핑크 니트 & 버건디 와이드 팬츠' },
        { mbti: 'ENFP', title: 'SEOUL AFTER HOURS', img: 'assets/editorial-rooftop.jpg', desc: '서로 다른 취향이 만나는 서울의 오후' },
        { mbti: 'ISTP', title: 'TACTICAL CARGO UTILITY', img: 'assets/editorial-orange.jpg', desc: '유틸리티 베스트 & 청키 바머' }
    ];

    track.innerHTML = items.map(look => `
        <div class="lookbook-card">
            <img loading="lazy" decoding="async" src="${look.img}" class="lookbook-card-img" alt="${look.title}">
            <div class="lookbook-overlay">
                <span class="lookbook-mbti-tag">${look.mbti} OUTFIT</span>
                <div class="lookbook-info">
                    <h4>${look.title}</h4>
                    <p>${look.desc}</p>
                    <button class="btn btn-adv-orange btn-full" onclick="openTypeArchive('${look.mbti}')">
                        EXPLORE THIS TYPE <i class="fa-solid fa-arrow-right"></i>
                    </button>
                </div>
            </div>
        </div>
    ...
    grid.innerHTML = items.map(p => `
        <div class="product-card-adv">
            <div class="product-thumb">
                <img loading="lazy" decoding="async" src="${p.img}" class="product-img" alt="${p.name}">
...
function renderCommunityFeed() {
    ...
    grid.innerHTML = feeds.map(f => `
        <div class="comm-card">
            <img loading="lazy" decoding="async" src="${f.img}" class="comm-img" alt="${f.user}">
            <div class="comm-body">
    ...
    container.innerHTML = cart.map((item, idx) => {
        ...
        return `
            <div style="display:flex; gap:1rem; align-items:center; background:var(--adv-dark-gray); border:var(--border-dim); padding:0.8rem; border-radius:8px; margin-bottom:0.8rem;">
                <img loading="lazy" decoding="async" src="${item.img}" style="width:50px; height:50px; object-fit:cover; border-radius:6px;">
